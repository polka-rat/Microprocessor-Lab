#include "../mcc.h" // subscribe node: receives subscribed (default topic) & publish & forget 
#include "../winc/m2m/m2m_wifi.h" // received (a) "hello" mssge (b) sensor data displayed in LCD
#include "../winc/socket/socket.h" // default topic is "mchp/iot/events" held by CFG_PUBTOPIC
...
static const char mqttPublishTopic[] = EE2016IITM; // = CFG_PUBTOPIC (default)
//const char mqttPublishMsg[] = "mchp payload"; mchp/iot/events
char mqttPublishMsg[] = "Welcome to EE2016IITM"; // "mchp payload";
...
uint8_t ESB106[] = "mchp/iot/events"; // subscribes to default topic CFG_PUBTOPIC of publish board A? 
uint8_t only_once = 0; // the above topic t[] is not default?? EE2016IITM

volatile uint8_t last_received_topic[PUBLISH_TOPIC_SIZE]; // from default topic published by A 
volatile uint8_t last_received_message[PAYLOAD_SIZE]; // message sent by board A
// above two lines are needed to interface with LCD
void create_MQTT_subscribe_packet(char *topic) { // topic "EE2016IITM"
...
} // core of subscribe node EE2016IITM
...
void MQTT_publish_handler_cb(uint8_t *topic, uint8_t *payload) { // called in scheduler
    ...
    printf("Received Message: %s\n", payload); // define a fn to display in LCD, call 
    PORTD.OUT = PORTD.OUT | 0xFB; // that fn. Also, similarly the light sensor data 
    delay (3000);// Board A publishes to default topic (mchp/iot/events), Board B 
    PORTD.OUT = PORTD.OUT | 0xF4; //  subscribes to this, default topic & receives 
    // and displays in the LCD. Board B also independently publishes to a different
    // topic "EE2016IITM", "Welcome to EE2016IITM" which we 
    // choose NOT to MONITOR. 
}   // this function is unique to subscribe node
...
static void app_buildMQTTConnectPacket(void) {// called by app_mqttExampleInit()
    ...
    appConnectPacket.clientID = "SubscribeBoardB";
...
static void app_buildPublishPacket(void) { // called by app_mqttScheduler()
    ...
    // this creates the publish packets (topic "EE2016IITM", mssge "Welcome to EE2016IITM")
    MQTT_CreatePublishPacket(&appPublishPacket); // in subscribe nodes EE2016
}   // this creates the publish packets in publish & subscribe nodes EE2016
...
static void wifiHandler(uint8_t u8MsgType, void *pvMsg) {
    ...
    // common for both publish & subscribe nodes
    switch (u8MsgType) {
...
static void dnsHandler(uint8_t *pu8DomainName, uint32_t u32ServerIP) {
    if (u32ServerIP) { // called by scheduler EE2016IITM
        if (appState == APP_STATE_WAIT_FOR_DNS) {
            changeState(APP_STATE_TLS_START); // switch to next state: EE2016IITM
...
void app_mqttScheduler(uint8_t* message_pointer1) {
    ...
    switch (appState) {
        ...
        {
            socketInit(); // socketHandler is-itn't a function?? EE2016IITM
        ...
        case SUBSCRIBING:
        {
            if (appMQTTPublishTimeoutOccured == true) //checks if timer was completed
            {
                ...
                if (only_once == 3) { // if timer was completed 3 times, subscribe
                    create_MQTT_subscribe_packet(ESB106); // core part. in subscribe, publish is not 
                    MQTT_TransmissionHandler(mqttConnnectionInfo); // removable EE2016IITM
                    only_once = 4; // to avoid only_once overflow and reset to be 3 again
                } // the 'ESB106' here is topic, user defined in line 51
            }
        ...
        case PUBLISHING:
        {
            ...
            changeState(PROCESS_LAST_PACKET); // MQTT transmit (publish) in topic "EE2016IITM"
            break;
        }
